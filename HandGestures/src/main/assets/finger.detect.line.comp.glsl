...
void findNextNeighbour(in ivec2 globalPos, inout vec2 lastPos, inout vec2 compoundDir, inout bool errorConHole, inout bool errorConLoop, inout bool errorManyNeighbours) {
    ...
    errorManyNeighbours = errorManyNeighbours || all(equal(nextNeighbour, vec4(MANY_NEIGHBOUR)));
}
...
void main() {
    ...
    if (any(notEqual(c, vec4(0.0)))) {
        ...
        //int index = atomicAdd(angleDebugBuff.index, 1);
        //angleDebugBuff.buff[index] = angleDegrees;
        if ((!(errorHole || errorLoop || errorManyNeighbours)) && angleThreshMin < angleDegrees && angleDegrees < angleThreshMax &&
         ...
         length(rghtCmp) > kMeansDistThresh) {
            ...
            vec2 shorterLine, longerLine;
            if (length(leftCmp) < length(rghtCmp)) {
                shorterLine = leftCmp; longerLine = rghtCmp;
            } else {
                longerLine = leftCmp; shorterLine = rghtCmp;
            }
            if (drawShortLine) drawLine(pos, shorterLine, colour);
            if (drawLongLine)  drawLine(pos, longerLine, colour);
