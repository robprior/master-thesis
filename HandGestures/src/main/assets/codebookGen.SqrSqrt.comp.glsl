...
void main() {
    ...
    uint minIdx = numCE;
    float I = sqrt(pix.r*pix.r + pix.g*pix.g + pix.b*pix.b);
    bool updatedCE = false;
    for (i = uint(0); i<numCE; ++i) {
        // if I lies within a code element update that element (extends ranges adds to usage metric)
        ...
        if (codeBook.data[offset+i].tLast == 0.0) {
            // gaurnteed to always be smaller than any acctual values
            minTlast = -1.0;
            minIdx = i;
            //break;
