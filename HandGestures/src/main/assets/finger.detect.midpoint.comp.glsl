    ...
    if (any(notEqual(c, vec4(0.0)))) {
        ...
        if (!errorManyNeighbours && angleThreshMin < angleDegrees && angleDegrees < angleThreshMax &&
            ...
            if (length(leftCmp) < length(rghtCmp)) {
                midpointPos = pos + ivec2((leftCmp/2.0));
            } else {
                midpointPos = pos + ivec2((rghtCmp/2.0));
            }
            drawBox(midpointPos, vec4(1.0, 1.0, 0.0, 1.0));
            kmeanLabel.labels[midpointPos.x * int(height) + midpointPos.y] = 1.0;
            atomicAdd(kmeanWeights.weights[midpointPos.x * int(height) + midpointPos.y], 1.0);
        } else {
            imageStore(outputBuffer, pos, vec4(0.0, 0.0, 1.0, 0.0)); // blue for high contrast
            kmeanLabel.labels[offset] = -2.0;
        }
    }
}
